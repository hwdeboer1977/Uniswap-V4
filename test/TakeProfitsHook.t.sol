...
// Our contracts
import {TakeProfitsHook} from "../src/LimitOrders/TakeProfitsHook.sol";
...
contract TakeProfitsHookTest is Test, Deployers {
    ...
    function setUp() public {
        ...
        // Deploy our 2 hooks: AFTER_INITIALIZE and AFTER_SWAP
    ...
    // As it turns out, the ERC-1155 implementation we're using from OpenZeppelin disallows
    // transferring to smart contracts unless smart contracts explicitly say they have support to handle them.
    // This would not be an issue if testing in Hardhat - for example - since Hardhat tests run through an EOA account
    // - but is an issue with Foundry.

    // To fix this, we need to add a couple of read-only functions to our test contract based
    // on the OpenZeppelin ERC-1155 implementation that makes it known that our contract has
    // support for being able to receive ERC-1155 tokens.
    ...
    function test_placeOrder() public {
        ...
        // Place the order: call placeorder function in hook contract
    ...
    // We will first place an order at Tick 100, i.e. a take-profit order to sell Token 0 when price increases.
    // Then, we'll conduct a swap to buy Token 0 (and sell Token 1) such that the tick of the pool increases
    // as Token 0 price increases. As a result, our limit order to sell Token 0 at the increased price
    // should execute as part of that swap inside afterSwap
    ...
    // This one is pretty much the same thing as the last - except in the opposite direction.
    // We place an order to sell Token 1 at tick -100. Then, we do a normal swap to buy Token 1
    // to make Token 1's price increase i.e. decrease the pool tick - and ensure our order was executed.
